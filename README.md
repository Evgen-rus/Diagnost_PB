...
└── 📖 Документация
    ...
    └── тестирования нейроконсультанта с использованием test_runner.md
```
...
- **Оценка качества** (-2 до +2): экспертная оценка правильности ответа
- **Количество токенов**: для оптимизации затрат на OpenAI API
- **Время генерации**: производительность системы
- **Стоимость запроса**: экономическая эффективность
- **Релевантность источников**: точность векторного поиска

### 🗄️ Структура данных тестирования

#### Таблица test_results (SQLite)
```sql
...
- **Назначение**: Фреймворк тестирования
- **Классы**:
  - `TestTable`: управление тестами
- **Функции**:
  - Проведение тестов
...
#### `test_runner.py`
- **Назначение**: Запуск системы тестирования
- **Функции**:
  - Одиночные тесты
  - Пакетное тестирование
  - Экспорт результатов
...
#### Добавление новых функций тестирования
1. Расширьте `utils/test_framework.py` новыми метриками
2. Обновите `test_runner.py` с новыми параметрами
3. Модифицируйте базу данных для хранения новых данных
...
# Тестирование бота
python test_runner.py --question "Тестовый вопрос"
```

### 📖 Документация кода

Все методы проекта содержат подробную документацию:
- Назначение метода
- Параметры и их типы
...
**Экспертные консультации**: Специалисты в области НК
...
**"Диагност ПБ"** является информационным помощником и предоставляет справочную информацию по неразрушающему контролю на основе доступных нормативных документов и технической литературы.

### 🚨 Важные ограничения

- **Справочный характер**: Все ответы носят исключительно информационный характер
- **Требуется проверка**: Обязательно сверяйтесь с актуальными официальными источниками
- **Консультации экспертов**: При принятии критических решений консультируйтесь со специалистами
- **Актуальность документов**: Проверяйте действующие редакции ГОСТов и стандартов
- **Локальные требования**: Учитывайте специфические требования вашего предприятия

### 📋 Рекомендации по использованию

1. **Используйте как отправную точку** для поиска информации
2. **Проверяйте все данные** в официальных источниках
3. **Консультируйтесь с экспертами** по сложным вопросам
4. **Учитывайте контекст** вашей конкретной задачи
5. **Следуйте корпоративным стандартам** вашей организации

### ⚖️ Ответственность

Разработчики и операторы системы **не несут ответственности** за:
- Решения, принятые на основе информации от бота
- Ущерб от использования предоставленной информации  
- Актуальность и полноту информации в базе знаний
- Соответствие информации локальным требованиям

**Пользователь самостоятельно несет ответственность** за проверку информации и принятие решений в области промышленной безопасности.

## 🆘 Поддержка

При возникновении проблем:

1. **Проверьте документацию**: README.md, INSTALL.md, HYBRID_VECTOR_SEARCH_GUIDE.md
2. **Просмотрите логи**: `journalctl -u diagnost-pb-bot -n 100`
3. **Протестируйте компоненты**: используйте скрипты test_*.py
4. **Проверьте конфигурацию**: убедитесь в правильности .env файла

---

**Диагност ПБ** - современное решение для специалистов неразрушающего контроля, объединяющее передовые технологии AI с глубокими знаниями в области промышленной безопасности. 